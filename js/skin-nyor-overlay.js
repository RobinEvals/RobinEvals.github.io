 ...
 *   "var": 1.000,        // 变体 0-1 (1.0=细腻, 0.5=中等, 0.0=粗糙)
 *   "g": 0               // 性别 0=雄, 1=雌
 * }
 *
 * 颜色空间: 线性 RGB，0-1 浮点
...
// patternScale ↔ var 映射
// var=1.0 → fine(细腻), var=0.5 → medium(中等), var=0.0 → coarse(粗糙)
const SCALE_TO_VAR = { 'fine': 1.0, 'medium': 0.5, 'coarse': 0.0 };

function varToScale(v) {
    const digit = Math.round((1.0 - v) * 2);
...
// pattern 名称 ↔ 数字索引
// Pattern_A → 0, Pattern_B → 1, Pattern_C → 2 ...
 ...
 * @param {string} params.pattern - Pattern_A / Pattern_B / Pattern_C ...
 ...
 */
export function encodeNyorOverlay({ isFemale, pattern, patternScale, varValue, colors, eyeColor }) {
    ...
    return `{"v":1,"c":[${c}],"p":${patternToIndex(pattern)},"var":${f3(varNum)},"g":${isFemale ? 1 : 0}}`;
}
